...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 17, 2016 at 03:49 PM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `Accepted_Cancel`(IN `bkid` VARCHAR(15), IN `usid` VARCHAR(15), OUT `usid2` VARCHAR(15))
BEGIN
    ...
    IF (cou=1)THEN
        ...
        IF (F!=-1)THEN
        	SET @sql = CONCAT('SELECT c',F,' INTO usid2 FROM bookrequests WHERE mainid ="',bkid,'"');
        	PREPARE stmt FROM @sql;
        	EXECUTE stmt;
        	DEALLOCATE PREPARE stmt;
            
            INSERT INTO notification (userid,bookid,not_date,type) VALUES(usid2,bkid,now()+1,'accepted');
...
('SCS13218', 2, 'CSES612901', '', '', 'CSES6103', '', '', '', ''),
('SCS14001', 0, '', '', '', '', '', '', '', ''),
